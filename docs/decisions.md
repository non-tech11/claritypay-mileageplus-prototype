...
13. **The earn model is final as built: 1 mi/$ of fare, 0.5 bonus mi/$ financed, and 0% earns nothing.** Base is 1 mi/$ of fare excluding taxes; the pay-over-time bonus is 0.5 mi/$ financed, capped at 1,000, on Economy Plus fares only, and identical on every APR term. A 0% plan writes no ledger entries at all — its subsidised rate is already the reward, and there is no margin left to fund miles on top of it. This supersedes the flat "500 bonus per booking / base 5 mi/$" sketch in the original build plan (`docs/PLAN.md`), which was written before the unit economics were worked through.
